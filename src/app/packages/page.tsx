        ...
        <div className="mx-auto max-w-5xl space-y-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">Choose the plan that fits your needs. Switch anytime — existing data is preserved.</p>
            </div>
              ...
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={cn(
                      "card flex flex-col gap-5 transition-all duration-300 glass-hover",
                      isActive && "ring-2 ring-indigo-500/60 shadow-lg shadow-indigo-500/10",
                    )}
                  >
                    {/* Header */}
                    <div>
                      <div className={cn("mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br", gradient)}>
                        <Icon size={18} className="text-white" />
                      </div>
                      <div className="flex items-center gap-2">
                        <h2 className="text-lg font-bold text-white">{item.name}</h2>
                        {isActive && <span className="badge bg-indigo-500/20 text-indigo-300 text-[10px]">Active</span>}
                      </div>
                    ...
                    <ul className="flex-1 space-y-2 text-xs">
                      {[
                        ...
                        <li key={label} className="flex items-center justify-between gap-2 rounded-lg bg-white/5 px-3 py-2">
                          <span className="flex items-center gap-1.5 text-slate-500"><I size={12} />{label}</span>
                    ...
                    {item.allowedFileTypes?.length ? (
                      <div className="flex flex-wrap gap-1">
                        {item.allowedFileTypes.map((t: string) => (
                          <span key={t} className="badge bg-slate-700/50 text-slate-400 text-[10px]">
                            {t.split("/")[1] ?? t}
                          </span>
                        ))}
                      </div>
                    ...
                    >
                      {isActivating ? (
                        ...
                        <span className="inline-flex items-center justify-center gap-1.5"><CheckCircle2 size={14} />Current Plan</span>
                      ) : (
                        "Activate Plan"
                      )}
                    </button>
