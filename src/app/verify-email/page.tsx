  ...
  return (
    <AuthLayout title="Verify your email" subtitle="Enter the token from your inbox">
      <div className="space-y-6">
        <form onSubmit={onResend} className="space-y-4 rounded-xl border border-white/10 bg-white/5 p-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-slate-500">Resend verification</p>
          <div className="relative">
            <Mail size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" />
            <input className="input-field pl-11" type="email" placeholder="Your email address" value={email} onChange={e => setEmail(e.target.value)} required />
          </div>
        ...
        <form onSubmit={onVerify} className="space-y-4 rounded-xl border border-white/10 bg-white/5 p-4">
          ...
          <div className="relative">
            ...
            <input className="input-field pl-11" placeholder="Verification token" value={token} onChange={e => setToken(e.target.value)} required />
          </div>
